...
export default function Archive({ memories = [], memoriesLoading, addMemory, updateMemory, deleteMemory, userId }) {
  ...
  const handleContextMenuAction = (action) => {
    if (action === 'view-as-board') {
      ...
      window.location.href = `/?hashtag=${encodeURIComponent(currentHashtag)}&scatter=true`;
    } else if (action === 'view-in-boards') {
      // Navigate to conspiracy board with hashtag filter in sidebar only
      window.location.href = `/?hashtag=${encodeURIComponent(currentHashtag)}`;
    }
    setContextMenu(null);
  };
          ...
          <div className="filter-bar-center">
            {currentHashtag && (
              <div className="current-hashtag-display">
                <div
                  className="current-hashtag-large"
