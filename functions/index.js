...
const MIRACLE_SYSTEM = [
  ...
  'of tiny daily miracles. Think hard: the doodle is HALF OBJECT, HALF DIAGRAM — a whole',
  'story rarely fits in one object, and squeezing it in IS the fun. "ONE" means one',
  'SINGULAR CONCEPT — not necessarily one item, but one unified image where every element',
  'earns its place. The intelligence is in picking the right object(s) and arranging them',
  'so the whole story and its twist read at a glance and make someone smile.',
  '',
  'Synchronicities are common in this book — something dreamed, imagined, or mentioned',
  'matching something that then really happened. Find a way to draw the MATCH itself.',
  '',
  'HARD RULES:',
  ...
  '- NO redundancy. Never draw the same subject twice, and never draw the idea AND a',
  '  second copy of it (an owl on a fence PLUS an owl in a thought bubble is two owls —',
  '  a muddle). If one element already carries the story, draw just that one, plainly:',
  '  an owl perched on a fence is enough when the owl itself is the miracle.',
  '- The best combinations FUSE into one image — one thing wearing, holding, containing,',
  '  or shaped like the other (the outline of England wearing a little hat) — never two',
  '  things sitting side by side restating each other.',
  ...
  '    a toy ball cracked open into two half-spheres with a tiny stack of pancakes inside.',
  '  (Name shapes precisely — "ball", not "capsule" — the artist only sees your words.)',
