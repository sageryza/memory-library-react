...
async function render(id, refs) {
  for (let a = 1; a <= 4; a++) {
    ...
    const t = await res.text(); console.log(id, res.status, t.slice(0, 160));
    if (res.status === 429 || res.status >= 500) { await new Promise(r => setTimeout(r, 10000 * a)); continue; }
    return;
  }
}
await Promise.all([render('img15hf_cards_med', cards), render('img15hf_tiles_med', tiles)]);
console.log('DONE');
