...
const vr = await fetch('https://api.replicate.com/v1/models/sageryza/sage-diagram-v2', { headers: { Authorization: `Bearer ${KEY}` } });
  ...
  for (let a = 1; a <= 5; a++) {
    const res = await fetch('https://api.replicate.com/v1/predictions', {
      ...
      body: JSON.stringify({ version: ver, input: {
        prompt: `SAGEDIAGRAM, ${m.c}, a simple hand-drawn doodle drawn large, thin black pen line on a plain white background, minimal, no color`,
...
for (let i = 0; i < CONCEPTS.length; i++) {
  ...
  svg += `<rect x="${x}" y="${y}" width="${CELL}" height="${CELL}" fill="none" stroke="#c9c1b2" stroke-width="2"/>`
    + `<text x="${x+CELL/2}" y="${y+CELL+24}" text-anchor="middle" font-family="sans-serif" font-size="18" fill="#2e2a24">${m.id} — sage-diagram-v2</text>`;
}
