...
console.log('total predict seconds:', secs.toFixed(1));
...
for (let i = 0; i < CONCEPTS.length; i++) {
  ...
  svg += `<rect x="${x}" y="${y}" width="${CELL}" height="${CELL}" fill="none" stroke="#c9c1b2" stroke-width="2"/>`
    + `<text x="${x+CELL/2}" y="${y+CELL+24}" text-anchor="middle" font-family="sans-serif" font-size="18" fill="#2e2a24">${m.id} — HOONIE linocut</text>`;
}
await sharp({ create: { width: W, height: H, channels: 3, background: '#f4efe6' } })
  ...
  .png().toFile(`${SP}/sheet_hoonie.png`);
