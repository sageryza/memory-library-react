...
export default function Chronology({ memories = [], memoriesLoading }) {
  ...
  useEffect(() => {
    ...
    // Don't reload from Firebase if we just made local changes (within 1 second)
    const timeSinceLocalUpdate = Date.now() - lastLocalUpdateRef.current;
    if (timeSinceLocalUpdate < 1000 && hasInitializedRef.current) {
      return;
    }
    ...
    if (dropTarget.type === 'gap') {
      ...
      if (draggedItem.isChunk) {
        ...
        if (draggedItem.fromSidebar) {
          ...
          // When dropping between two memories, we need to ensure gaps exist
          // Check what's at the drop position and around it
          const itemBefore = dropPosition > 0 ? newTimeline[dropPosition - 1] : null;
          const itemAt = newTimeline[dropPosition];

          // Determine what to insert based on surrounding items
          if (itemBefore?.type === 'memory' && itemAt?.type === 'memory') {
            // Dropping between two memories - insert gap, memory, gap
            newTimeline.splice(dropPosition, 0,
              { id: `gap-${Date.now()}-before`, type: 'gap' },
              memoryToAdd,
              { id: `gap-${Date.now()}-after`, type: 'gap' }
            );
          } else if (itemBefore?.type === 'memory') {
            // Only a memory before - insert gap then memory
            newTimeline.splice(dropPosition, 0,
              { id: `gap-${Date.now()}-before`, type: 'gap' },
              memoryToAdd
            );
          } else if (itemAt?.type === 'memory') {
            // Only a memory after - insert memory then gap
            newTimeline.splice(dropPosition, 0,
              memoryToAdd,
              { id: `gap-${Date.now()}-after`, type: 'gap' }
            );
          } else {
            // Gap or ghost on both sides - just insert the memory
