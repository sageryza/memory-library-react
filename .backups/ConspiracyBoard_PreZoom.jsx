...
const handleDragEnd = (event) => {
  ...
  if (over && (over.id === 'canvas' || over.id === 'canvas-container') && !memoryData?.isOnCanvas && !memoryData?.isStandalonePin) {
    ...
    // Transform to canvas coordinates by subtracting the pan offset
    const canvasX = screenX - panOffset.x
    const canvasY = screenY - panOffset.y
  ...
  const handleCanvasMouseMove = useCallback((e) => {
    if (isPlacingPin) {
      ...
      setPinPlacementPosition({
        x: e.clientX - containerRect.left - panOffset.x,
        y: e.clientY - containerRect.top - panOffset.y
      })
    }
  }, [isPlacingPin, panOffset])
  ...
  const handleCanvasClick = useCallback((e) => {
    ...
    if (isPlacingPin && pinPlacementPosition) {
      const containerRect = document.querySelector('.canvas-container').getBoundingClientRect()
      // Transform screen coordinates to canvas coordinates
      const x = e.clientX - containerRect.left - panOffset.x
      const y = e.clientY - containerRect.top - panOffset.y
